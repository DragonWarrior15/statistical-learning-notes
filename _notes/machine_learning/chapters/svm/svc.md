...
\begin{align}
    ...
    \end{align}
Replacing the constraint on the sum of the slack variables with a simple $C * sum$ is similar to the [alternative formulation of ridge and lasso regression]({{ "/notes/machine_learning/chapters/linear_model_selection_regularization/shrinkage.html#sec:alternative_ridge_lasso" | relative_url }}). $C$ is a constant and will limit how much error we are wiling to tolerate. $C = \inf$ gives us the original separable case since that forces all the $\epsilon$ to be $0$. As we decrease $C$, the epsilons are allowed to take larger values we are allowing more points to be misclassified leading to narrower margins (in figure below).
