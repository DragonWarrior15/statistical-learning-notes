...
$$
\begin{aligned}
    \text{orthogonal if } v_{i}^{T}v_{j} = \begin{cases} 1 &\mbox{$i = j$}\newline 0 &\mbox{otherwise} \end{cases} \: i,j = 1,2,\ldots, m\end{aligned}
$$
...
\begin{aligned}
    ...
    \Rightarrow 0 &= \alpha_{i}\end{aligned}
$$
...
\begin{aligned}
    ...
    v_{i}^{T}v &= \alpha_{i}\newline
    \Rightarrow v &= (v_{1}^{T}v)v_{1} + \cdots + (v_{m}^{T}v)v_{m}\end{aligned}
$$
