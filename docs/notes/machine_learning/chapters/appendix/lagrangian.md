...
$$
\begin{aligned}
    \minimize_{x} f(x) \quad \text{subject to} \quad h(x) = c\end{aligned}
$$
...
<figure class="notes-img">
  <img src="../../images/lagrange_1.png" alt="Gradients for level surfaces f(x,y) = x^{2}+y^{2}">
  <figcaption>Gradients for level surfaces \(f(x,y) = x^{2}+y^{2}\)</figcaption>
</figure>
...
\begin{aligned}
    ...
    \frac{\partial L}{\partial x}(x, \lambda) = 0 \quad \text{and}\quad \frac{\partial L}{\partial \lambda}(x, \lambda) = 0\end{aligned}
$$
...
\begin{aligned}
    ...
    L(x, \lambda, \mu) = f(x) + \lambda^{T}h(x) + \mu^{T}g(x)\end{aligned}
$$
...
$$
\begin{aligned}
    q(\lambda, \mu) = \min_{x} L(x, \lambda, \mu) \quad q:\mathbb{R}^{m+p} \rightarrow \mathbb{R}\end{aligned}
$$
...
\begin{aligned}
    ...
    \text{with} \quad \mu \geq 0 \quad \text{and} \quad q \text{ is bounded}\end{aligned}
$$
...
\begin{aligned}
    ...
    = f(x^{\*}) + \lambda^{T}h(x^{\*}) + \mu^{T}g(x^{\*}) \leq f(x^{\*})\end{aligned}
$$
...
{5}
    ...
    &\text{if} \quad g(x^{\*}) < 0 \quad &\text{then} \quad &\frac{df}{dx}(x^{\*}) +\lambda^{T} \frac{dh}{dx}(x^{\*}) + \mu^{T} \frac{dg}{dx}(x^{\*}) = 0 &\text{ and } &\mu = 0\end{aligned}
$$
...
$$
\begin{aligned}
    g(x^{\*}) \leq 0, \quad \mu \geq 0, \quad \mu^{T}g(x^{\*}) = 0\end{aligned}
$$
...
$$
\begin{aligned}
     \max_{\mu} q(\mu) = \max_{\mu} \min_{x} L(x, \mu)\end{aligned}
$$
...
\begin{aligned}
    ...
    \text{subject to} \quad \mu \geq 0 \quad \text{and} \quad \frac{\partial L}{\partial x}=0\end{aligned}
$$
