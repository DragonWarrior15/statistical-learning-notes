...
\begin{aligned}
    ...
    K(x_{i}, x_{j}) = \hat{K}(x_{i} - x_{j}), \quad m(x) = constant \quad \text{for stationarity}\end{aligned}
$$
...
\begin{aligned}
    ...
    \text{White Noise} &= \sigma^{2}I(x_{1} = x_{2})\end{aligned}
$$
...
<figure class="notes-img">
  <img src="../../images/gp_1.png" alt="Example of a 1d gaussian process. The orange line is the mean.">
  <figcaption>Example of a \(1d\) gaussian process. The orange line is the mean.</figcaption>
</figure>
...
\begin{aligned}
    ...
    f^{T} &= (f(x_{1}, f(x_{2}), \ldots, f(x_{N}))\end{aligned}
$$
...
$$
\begin{aligned}
    p(f(x) | f(x_{1}), \ldots, f(x_{N})) = \mathcal{N}(k^{T}C^{-1}f, K(0) - k^{T}C^{-1}k)\end{aligned}
$$
...
<figure class="notes-img">
  <img src="../../images/gp_2.png" alt="Prediction using gaussian process for a 1d distribution. Variance of the prediction is 0 at the observed points. The variance is constant far away from the observed points as K(0) is predefined and finite. Prepared using gaussian_process_prediction.py">
  <figcaption>Prediction using gaussian process for a \(1d\) distribution. Variance of the prediction is \(0\) at the observed points. The variance is constant far away from the observed points as \(K(0)\) is predefined and finite. Prepared using <a href="../../codes/gaussian_process_prediction.md">gaussian_process_prediction.py</a></figcaption>
</figure>
...
$$
\begin{aligned}
    \tilde{K}(x1- x2) = K(x1 - x2) + s^{2}I(x1 = x2)\end{aligned}
$$
...
\begin{aligned}
    ...
    = \max_{s^{2}, \sigma^{2}, l} \mathcal{N}(0, \tilde{K})\end{aligned}
$$
...
\begin{aligned}
    ...
    prediction &= \int p(y \mid f(x))p^{*}(f(x))\,d(f(x))\end{aligned}
$$
