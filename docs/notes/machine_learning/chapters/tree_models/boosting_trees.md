...
\begin{aligned}
    ...
    \text{A more simpler form} \quad R, \gamma = \argmin_{R, \gamma} \sum_{i=1}^{N} L(y_{i}, T(x_{i}; R, \gamma))\end{aligned}
$$
...
For classification problems with the exponential loss, this equation is same as the adaboost algorithm discussed in [adaboost](adaboost.md) with the restriction that the output of any tree is $\in \{-1,1\}$.
...
\begin{aligned}
    ...
    \mathbf{f}(x) &= \argmin_{f(x_{1}), f(x_{2}), \ldots, f(x_{N})} \sum_{i=1}^{N} L(y_{i}, f(x_{i}))\end{aligned}
$$
...
$$
\begin{aligned}
    \mathbf{f}\_{M}(x) = \sum_{m=1}^{M} \mathbf{h}\_{m}(x) \quad \text{where} \quad \mathbf{h}\_{m}(x) \in \mathbb{R}^{N}\end{aligned}
$$
...
$$
\begin{aligned}
    g_{i, m} = \bigg[ \frac{\partial L(y_{i}, f(x_{i}))}{\partial f(x_{i})} \bigg]\_{f=f_{m-1}}\end{aligned}
$$
...
\begin{aligned}
    ...
    \mathbf{f}\_{m} &= \mathbf{f}\_{m-1} - \rho_{m} g_{m}\end{aligned}
$$
...
$$
\begin{aligned}
    R, \gamma = \argmin_{r, \gamma} \sum_{i=1}^{N} (-g_{i,m} - T(x_{i}; R, \gamma))^{2}\end{aligned}
$$
...
$$
\begin{aligned}
    f_{m}(x) = f_{m-1}(x) + \nu \sum_{j=1}^{J} \gamma_{m,j} I(x \in R_{m,j})\end{aligned}
$$
...
\begin{aligned}
    ...
    R_{j} &= R_{j,1} \cup R_{j,2} \quad \text{is obtained by paritioning on variable $l$}\end{aligned}
$$
...
$$
\begin{aligned}
     \mathcal{I}\_{l}^{2}(T_{m}) = \sum_{j=1}^{J-1} \hat{i}\_{j} I(v(j) = l)\end{aligned}
$$
...
$$
\begin{aligned}
    \mathcal{I}\_{l}^{2} = \frac{1}{M}\sum_{m=1}^{M} \mathcal{I}\_{l}^{2}(T_{m})\end{aligned}
$$
