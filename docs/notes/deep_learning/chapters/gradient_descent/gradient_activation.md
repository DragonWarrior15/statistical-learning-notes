...
$$
\begin{aligned}
    \frac{dL}{dz} &= \frac{dL}{dp}\odot p\odot (1-p)\end{aligned}
$$
...
$$
\begin{aligned}
    s &= \bigg( \frac{exp(z_{1})}{\sum_{i=1}^{k}\exp(z_{i})}, \frac{exp(z_{2})}{\sum_{i=1}^{k}\exp(z_{i})}, \ldots, \frac{exp(z_{k})}{\sum_{i=1}^{k}\exp(z_{i})} \bigg)^{T}\end{aligned}
$$
...
$$
\begin{aligned}
    \frac{ds}{dz} = diag(s) - ss^{T} = J\end{aligned}
$$
...
\begin{aligned}
    ...
    &= s_{i}\frac{dL}{ds_{i}} - s_{i} \bigg( \sum_{j=1}^{k}\frac{dL}{ds_{j}}s_{j} \bigg)\end{aligned}
$$
...
$$
\begin{aligned}
    \sum_{j=1}^{k}\frac{dL}{ds_{j}}s_{j} = s^{T}\frac{dL}{ds} = c\end{aligned}
$$
...
\begin{aligned}
    ...
    &= \bigg( \frac{dL}{ds} - s^{T}\frac{dL}{ds} \bigg) \odot s\end{aligned}
$$
...
\begin{aligned}
    ...
    \frac{dL}{dz} &= \bigg( \frac{dL}{ds} - \bigg(\frac{dL}{ds} \odot s \bigg)\boldsymbol{1}\_{k} \bigg) \odot s\end{aligned}
$$
...
$$
\begin{aligned}
    z = tanh(x) = \frac{exp(x) - exp(-x)}{exp(x) + exp(-x)} = \frac{exp(2x) - 1}{exp(2x) - 1}\end{aligned}
$$
...
$$
\begin{aligned}
    \frac{dz}{dz} = \frac{(exp(x) + exp(-x))^{2} - (exp(x) - exp(-x))^{2}}{(exp(x) + exp(-x))^{2}} = 1 - tanh^{2}(x)\end{aligned}
$$
...
\begin{aligned}
    z &= tanh(x)\newline
    \frac{dL}{dX} &= \frac{dL}{dz} \odot (1 - tanh^{2}(x)) = \frac{dL}{dz} \odot (1 - z^{2})\end{aligned}
$$
