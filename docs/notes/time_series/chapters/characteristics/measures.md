...
$$
\begin{aligned}
    F(x_{1}, x_{2}, \ldots, x_{n}) = P(x_{1} \leq c_{1}, x_{2} \leq c_{2}, \ldots x_{n} \leq c_{n})\end{aligned}
$$
...
\begin{aligned}
    F_{t}(x) &= P(x_{t} \leq x)\newline
    f_{t}(x) &= \frac{\partial F_{t}(x)}{\partial x}\end{aligned}
$$
...
$$
\begin{aligned}
    \mu_{xt} = E[x_{t}] =  \int_{-\infty}^{\infty} x f_{t}(x) dx\end{aligned}
$$
...
\begin{aligned}
    ...
    E[v_{t}] &= \frac{1}{3}(3E[x_{t}]) = E[x_{t}]\end{aligned}
$$
...
$$
\begin{aligned}
    \gamma_{x}(s, t) = E[(x_{s} - \mu_{s})(x_{t} - \mu_{t})]\end{aligned}
$$
...
\begin{aligned}
    ...
    &= \frac{3}{9} \sigma_{w}^{2}\end{aligned}
$$
...
\begin{aligned}
    ...
    &= \frac{2}{9}\sigma_{w}^{2}\end{aligned}
$$
...
\begin{aligned}
    ...
    \gamma(s,t) = \begin{cases} \frac{3}{9}\sigma_{w}^{2} &\mbox{$s = t$},\newline
                                ...
                                0 &\mbox{otherwise} \end{cases}\end{aligned}
$$
...
$$
\begin{aligned}
    \rho(s,t) = \frac{\gamma_{x}(s,t)}{\sqrt{\gamma_{x}(s,s) \gamma_{x}(t,t)}}\end{aligned}
$$
...
\begin{aligned}
    ...
    \rho_{xy}(s,t) = \frac{\gamma_{xy}(s,t)}{\sqrt{\gamma_{x}(s,s) \gamma_{y}(t,t)}}\end{aligned}
$$
...
$$
\begin{aligned}
    P(x_{t} \leq c) = P(x_{s} \leq c)\end{aligned}
$$
...
$$
\begin{aligned}
    P(x_{t} \leq c_{1}, x_{s} \leq c_{2}) = P(x_{t+h} \leq c_{1}, x_{s+h} \leq c_{2})\end{aligned}
$$
...
$$
\begin{aligned}
    \gamma (h) = \gamma(t+h,t) = E[(x_{t+h} - \mu)(x_{t} - \mu)]\end{aligned}
$$
...
$$
\begin{aligned}
    \rho (h) = \rho(t+h, t) = \frac{\gamma (t+h,t)}{\sqrt{\gamma(t+h, t+h) \gamma(t, t)}} = \frac{\gamma(h)}{\gamma(0)}\end{aligned}
$$
...
$$
\begin{aligned}
    PACF(X_{t},X_{t+h}) = \frac{ACF(X_{t},X_{t+h}|X_{t+1},X_{t+2},\ldots,X_{t+h-1})}{\sqrt{Var(X_{t}|X_{t+1},X_{t+2},\ldots,X_{t+h-1})Var(X_{t+h}|X_{t+1},X_{t+2},\ldots,X_{t+h-1})}}\end{aligned}
$$
...
\begin{aligned}
    ...
    &= \frac{Cov(X_{t} - \beta_{1}X_{t-1}, X_{t-2} - \beta_{2}X_{t-1})}{\sqrt{Var(X_{t} - \beta_{1}X_{t-1})Var(X_{t-2} - \beta_{2}X_{t-1})}}\end{aligned}
$$
...
$$
\begin{aligned}
    \gamma_{xy}(h) = Cov(x_{t+h}, y_{t}) = E[(x_{t+h} - \mu_{x})(y_{t} - \mu_{y})]\end{aligned}
$$
...
$$
\begin{aligned}
    \rho_{xy}(h) = \frac{\gamma_{xy}(h)}{\sqrt{\gamma_{x}(0) \gamma_{y}(0)}}\end{aligned}
$$
...
$$
\begin{aligned}
    x_{t} = w_{t} + w_{t-1} \quad y_{t} = w_{t} - w_{t-1}\end{aligned}
$$
...
\begin{aligned}
    ...
    \gamma_{xy}(h) &= \begin{cases} 0 &\mbox{$h = 0$}\newline 1/2 &\mbox{$h=1$}\newline -1/2 &\mbox{$h=1$}\newline 0 &\mbox{$\lvert h \rvert \geq 2$} \end{cases}\end{aligned}
$$
...
$$
\begin{aligned}
    \bar{x} = \frac{1}{n} \sum_{i=1}^{n} x_{t}\end{aligned}
$$
...
$$
\begin{aligned}
    Var(x) = E[x^{2}] - E[x]^{2} = \frac{1}{n} \bigg( \sum_{i=1}^{n} x_{t}^{2} \bigg) - \bigg( \frac{1}{n}\sum_{s=1}^{n}x_{s} \bigg)^{2}\end{aligned}
$$
...
$$
\begin{aligned}
    \gamma_{x}(h) = Cov(x_{t+h},x_{t}) = \frac{1}{n}\sum_{i=1}^{n-h}x_{i+h}x_{i}\end{aligned}
$$
...
$$
\begin{aligned}
    \hat{\rho}(h) = \frac{\hat{\gamma}\_{x}(h)}{\hat{\gamma}\_{x}(0)}\end{aligned}
$$


<figure class="notes-img">
  <img src="../../images/acf_1.png" alt="Top figure shows a time series, with the theoretical and estimated ACFs shown by middle and bottom figures respectively. Due to the symmetric nature, most packages only plot ACF for positive lags. Figures plot using acf_simulation.py">
  <figcaption>Top figure shows a time series, with the theoretical and estimated ACFs shown by middle and bottom figures respectively. Due to the symmetric nature, most packages only plot ACF for positive lags. Figures plot using <a href="../../codes/acf_simulation.md">acf_simulation.py</a></figcaption>
</figure>

<figure class="notes-img">
  <img src="../../images/pacf_1.png" alt="Top figure shows a time series, with the sample ACF and sample PACFs shown by middle and bottom figures respectively. Due to the symmetric nature, most packages only plot ACF for positive lags. Figures plot using pacf_simulation.py">
  <figcaption>Top figure shows a time series, with the sample ACF and sample PACFs shown by middle and bottom figures respectively. Due to the symmetric nature, most packages only plot ACF for positive lags. Figures plot using <a href="../../codes/pacf_simulation.md">pacf_simulation.py</a></figcaption>
</figure>
